...
import { searchYouTubeV2 } from '../src/services/youtube.v2';
...
describe('youtube service (v2) - edge cases', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('handles partial JSON data across multiple chunks', async () => {
    ...
    const searchPromise = searchYouTubeV2('test');
  ...
  it('flushes multiple buffered lines on close (no trailing newline)', async () => {
    ...
    // No trailing newline at the end → both should be processed on close.
